...
public class EssayClientController {
    ...
    public BaseResponse addEssayClient(@RequestBody CreateEssayClientRequest request) {
    ...
    @PreAuthorize("hasRole('QM_ADMIN')")
    public BaseResponse deleteEssayClient(@RequestBody UpdateEssayClientRequest request) {
        essayClientService.deleteEssayClient(request.getClientId());
        return new BaseResponse(true, null);
    }

    @PostMapping("/updateClient")
    @PreAuthorize("hasRole('QM_ADMIN')")
    public BaseResponse updateEssayClient(@RequestBody UpdateEssayClientRequest request) {
        try {
            int updatedRows = essayClientService
                    .updateEssayClient(request.getClientId(), request.getName(), request.getDownloadCap());
            if (updatedRows <= 0)
                return new BaseResponse(false, "Failed to update, please check the client id.");
            else
                return new BaseResponse(true, null);
        } catch(Exception e) {
    ...
    public String listActiveClients() {
        return essayClientService.listActiveEssayClients().toPrettyString();
    }

//    @PostMapping("/changeEvent")
//    @PreAuthorize(("hasRole('QM_ADMIN')"))
//    public BaseResponse changeEvent(@RequestBody ChangeEventRequest request) {
//
//        // Rest of your logic...
//
...
//    }
    ...
    public BaseResponse changeEvent(@RequestBody ChangeEventRequest request) {
        ...
        System.out.println("Rule Name: " + request.getRuleName());
        System.out.println("Client Name: " + request.getClientName());
        System.out.println("Start: " + request.isStart());
        ...
        try {
            ...
            System.out.println("Cognito sub i.e. person " + cognitoSub);
