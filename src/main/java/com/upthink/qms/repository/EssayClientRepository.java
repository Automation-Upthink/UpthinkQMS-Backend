...
public interface EssayClientRepository extends JpaRepository<EssayClient, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE essay_client SET deleted_at = CURRENT_TIMESTAMP WHERE id = :id", nativeQuery = true)
    void softDeleteById(Integer id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE essay_client SET name = :name, download_capacity = :downloadCapacity WHERE id = :id", nativeQuery = true)
