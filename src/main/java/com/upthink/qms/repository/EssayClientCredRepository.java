...
public interface EssayClientCredRepository extends JpaRepository<EssayClientCred, Integer> {
    ...
    @Query(value = "SELECT * FROM EssayClientCred  WHERE name = :name", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE essay_client_credential SET deleted_at = CURRENT_TIMESTAMP WHERE id = :id", nativeQuery = true)
    EssayClientCred deleteEssayClientCred(@Param("id") Integer id);
